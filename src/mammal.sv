...
module mammal (
     ...
     always_ff @(posedge clk) begin
          case(state)
               FETCH: begin
                    if ({1'b0, data_in[15:12]} == JZ) begin
                         if (zeroflag)
                              state <= JMP;
                         else
                              state <= FETCH;
               ...
               INT2: begin
                    regbank[7] <= regbank[7] - 1;
                    interruptreg <= 12'(data_in + 16'h01f0); // Address of IDT is changed as 16'h01f0
                    state <= INT3;
               end
               INT3: begin
                    pc <= data_in[11:0];
     ...
     always_comb begin
          case (state)
               LD, ST, PUSH, POP2, CALL, RET2, IRET1, IRET2, IRET3: address = regbank[7][11:0];
               INT1, INT2: address = regbank[7][11:0];
